...
pub fn elaborate_fn(db: &dyn salsa::Database, program: Program, func: FnDecl) -> Option<TypedExpr> {
    ...
    elaborate_expr(db, program, &var_env, &type_env, func.body(db))
}
...
fn elaborate_expr(
    ...
    match expr {
        ...
        crate::ast::Expr::BinOp(op, lhs, rhs) => {
            let lhs = elaborate_expr(db, program, var_env, type_env, lhs)?;
            let rhs = elaborate_expr(db, program, var_env, type_env, rhs)?;
            Some(TypedExpr {
        ...
        crate::ast::Expr::Call(name, args) => {
            ...
            let elaborated: Option<Vec<TypedExpr>> = args
                .iter()
                .map(|a| elaborate_expr(db, program, var_env, type_env, a))
                .collect();
            Some(TypedExpr {
                kind: TypedExprKind::Call(name.clone(), elaborated?),
                ty: sig.return_type,
            })
        }
    }
