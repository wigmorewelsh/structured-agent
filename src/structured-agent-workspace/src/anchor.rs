...
pub fn strip_annotations(annotated: &str) -> String {
    annotated
        .lines()
        .map(|line| {
            line.split_once('|')
...
pub enum PatchError {
    ...
    #[error("start anchor comes after end anchor")]
    AnchorOrderInvalid,
}
...
mod tests {
    ...
    #[test]
    fn strip_removes_hash_prefixes() {
        let annotated = annotate("foo\nbar\nbaz");
        assert_eq!(strip_annotations(&annotated), "foo\nbar\nbaz");
    }

    #[test]
    fn strip_roundtrips_with_annotate() {
        let original = "fn main() {\n    println!(\"hello\");\n}";
        assert_eq!(strip_annotations(&annotate(original)), original);
    }
