...
use syn::{Attribute, Block, FnArg, ItemFn, Pat, ReturnType, Type as SynType};
...
fn build_param_constructions(params: &[(Ident, SynType)]) -> syn::Result<Vec<TokenStream2>> {
    params
        .iter()
        .map(|(name, ty)| {
            let name_str = name.to_string();
            let rt = map_type_to_runtime(ty)?;
            Ok(quote! { ::structured_agent_runtime::Parameter::new(#name_str.to_string(), #rt) })
        })
        .collect()
}

fn build_arg_extractions(params: &[(Ident, SynType)]) -> syn::Result<Vec<TokenStream2>> {
    params
        .iter()
        .enumerate()
        .map(|(i, (name, ty))| {
            let extraction = extract_arg(i, ty)?;
...
fn build_return_conversion(ret_ty: Option<&SynType>, body: &Block) -> syn::Result<TokenStream2> {
    ...
    let conv = convert_result(ret_ty.unwrap())?;
    Ok(quote! {
...
pub fn generate_native_function(input: ItemFn) -> syn::Result<TokenStream2> {
    ...
    let param_constructions = build_param_constructions(&params)?;
    ...
    let return_conversion = build_return_conversion(ret_ty, &input.block)?;
    let doc_fn = doc
